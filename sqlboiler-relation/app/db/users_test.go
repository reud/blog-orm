...
func testUserToManySetOpCreditCards(t *testing.T) {
	var err error
	...
	err = a.SetCreditCards(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetCreditCards(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.CreditCards[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.CreditCards[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpCreditCards(t *testing.T) {
	var err error
	...
	err = a.AddCreditCards(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveCreditCards(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.CreditCards) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.CreditCards[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.CreditCards[0] != &e {
