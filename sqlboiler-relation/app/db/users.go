...
func (o *User) CreditCards(mods ...qm.QueryMod) creditCardQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"credit_card\".\"user_id\"=?", o.ID),
	)

	query := CreditCards(queryMods...)
	queries.SetFrom(query.Query, "\"credit_card\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"credit_card\".*"})
	}
...
func (userL) LoadCreditCards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`credit_card`), qm.WhereIn(`credit_card.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load credit_card")
	}

	var resultSlice []*CreditCard
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice credit_card")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on credit_card")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for credit_card")
	}
...
// AddCreditCardsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreditCards.
// Sets related.R.User appropriately.
// Uses the global database handle.
func (o *User) AddCreditCardsG(ctx context.Context, insert bool, related ...*CreditCard) error {
	return o.AddCreditCards(ctx, boil.GetContextDB(), insert, related...)
}

// AddCreditCardsP adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreditCards.
// Sets related.R.User appropriately.
// Panics on error.
func (o *User) AddCreditCardsP(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CreditCard) {
	if err := o.AddCreditCards(ctx, exec, insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// AddCreditCardsGP adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreditCards.
// Sets related.R.User appropriately.
// Uses the global database handle and panics on error.
func (o *User) AddCreditCardsGP(ctx context.Context, insert bool, related ...*CreditCard) {
	if err := o.AddCreditCards(ctx, boil.GetContextDB(), insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}
...
// SetCreditCardsG removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's CreditCards accordingly.
// Replaces o.R.CreditCards with related.
// Sets related.R.User's CreditCards accordingly.
// Uses the global database handle.
func (o *User) SetCreditCardsG(ctx context.Context, insert bool, related ...*CreditCard) error {
	return o.SetCreditCards(ctx, boil.GetContextDB(), insert, related...)
}

// SetCreditCardsP removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's CreditCards accordingly.
// Replaces o.R.CreditCards with related.
// Sets related.R.User's CreditCards accordingly.
// Panics on error.
func (o *User) SetCreditCardsP(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CreditCard) {
	if err := o.SetCreditCards(ctx, exec, insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetCreditCardsGP removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's CreditCards accordingly.
// Replaces o.R.CreditCards with related.
// Sets related.R.User's CreditCards accordingly.
// Uses the global database handle and panics on error.
func (o *User) SetCreditCardsGP(ctx context.Context, insert bool, related ...*CreditCard) {
	if err := o.SetCreditCards(ctx, boil.GetContextDB(), insert, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetCreditCards removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's CreditCards accordingly.
// Replaces o.R.CreditCards with related.
// Sets related.R.User's CreditCards accordingly.
func (o *User) SetCreditCards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CreditCard) error {
	query := "update \"credit_card\" set \"user_id\" = null where \"user_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.CreditCards {
	...
	return o.AddCreditCards(ctx, exec, insert, related...)
}

// RemoveCreditCardsG relationships from objects passed in.
// Removes related items from R.CreditCards (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
// Uses the global database handle.
func (o *User) RemoveCreditCardsG(ctx context.Context, related ...*CreditCard) error {
	return o.RemoveCreditCards(ctx, boil.GetContextDB(), related...)
}

// RemoveCreditCardsP relationships from objects passed in.
// Removes related items from R.CreditCards (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
// Panics on error.
func (o *User) RemoveCreditCardsP(ctx context.Context, exec boil.ContextExecutor, related ...*CreditCard) {
	if err := o.RemoveCreditCards(ctx, exec, related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// RemoveCreditCardsGP relationships from objects passed in.
// Removes related items from R.CreditCards (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
// Uses the global database handle and panics on error.
func (o *User) RemoveCreditCardsGP(ctx context.Context, related ...*CreditCard) {
	if err := o.RemoveCreditCards(ctx, boil.GetContextDB(), related...); err != nil {
		panic(boil.WrapErr(err))
	}
}

// RemoveCreditCards relationships from objects passed in.
// Removes related items from R.CreditCards (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveCreditCards(ctx context.Context, exec boil.ContextExecutor, related ...*CreditCard) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.UserID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.CreditCards {
			if rel != ri {
				continue
			}

			ln := len(o.R.CreditCards)
			if ln > 1 && i < ln-1 {
				o.R.CreditCards[i] = o.R.CreditCards[ln-1]
			}
			o.R.CreditCards = o.R.CreditCards[:ln-1]
			break
		}
	}
